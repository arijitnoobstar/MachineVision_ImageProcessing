...
% This code handles the image processing operations for Image 2. To anybody
...
segmented_image_length = 41; % length of segmented image (as a square image)
...
im = imread("charact2.jpg");
% show the raw image before inversion
figure(1)
display_image(im, "Image before inversion");
% As the image is in RGB format, we convert it to grayscale for analysis in
% this project
im = rgb2gray(im);
rows = size(im, 1); % 77
cols = size(im, 2); % 173
...
% Before proceeding the image processing for Image 2, we much invert the
% characters. We can just use the in-built matlab function "imrotate" to rotate the im 
% by 180 degrees. However in this project, we strive to do as many operations without relying on 
% in-built functions. The following is the implementation for rotating.

% We create a dummy array to store the pixel values of the inverted image
im_temp = zeros(rows, cols, 'uint8');
% Then we loop through the current image and insert the pixel value in the
% 180 degrees flip, hence using the opposite i and j indices for im_temp
for i = 1:rows
    for j = 1:cols
        im_temp(rows-i+1, cols-j+1) = im(i,j);
    end
end
...
for lbl = 1:number_of_components
    ...
    % HARDCODED FOR charact2.jpg to remove non-character components (the
    % symbol a t the top left)
    if lbl == 2 || lbl == 8
        continue
    end
...
% Character 4
segmented_im_ordered = [segmented_im_ordered cell2mat(segmented_im(10))];
% Character 4
segmented_im_ordered = [segmented_im_ordered cell2mat(segmented_im(11))];
% Character 1
segmented_im_ordered = [segmented_im_ordered cell2mat(segmented_im(12))];
% Character 0
segmented_im_ordered = [segmented_im_ordered cell2mat(segmented_im(6))];
% Character 0
segmented_im_ordered = [segmented_im_ordered cell2mat(segmented_im(13))];
% Character A
...
% Character H
segmented_im_ordered = [segmented_im_ordered cell2mat(segmented_im(14))];
% Character D
segmented_im_ordered = [segmented_im_ordered cell2mat(segmented_im(9))];
% Character F
segmented_im_ordered = [segmented_im_ordered cell2mat(segmented_im(7))];
% Character S
segmented_im_ordered = [segmented_im_ordered cell2mat(segmented_im(8))];
